...
| `@deepseek-ai/dsh-tool-text` | `text_stats` | `ctx.tools` | `tool/call`, `tool/result` | - | text_stats is a pure, stateless measurement with no Config and no session or agent coupling; the catalog entry states the exact line/word/character/byte semantics. |
...
Compute line, word, character, non-whitespace, and UTF-8 byte counts of a text value.
...
Source: [`packages/text/tool-text/src/index.ts`](../packages/text/tool-text/src/index.ts)
