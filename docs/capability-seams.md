...
| `ctx.desktopRuntime` | `core` | [`desktop-app`](../packages/bundle/desktop-app) | - | `desktop` | - | Publishes the built frontend dist location for the privileged dsh: protocol handler; the Electron main process resolves it instead of mounting an HTTP server. |
