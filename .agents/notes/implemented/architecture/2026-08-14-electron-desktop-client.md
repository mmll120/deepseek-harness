...
Connection tests select `ElectronApiClient` on `dsh:`, keep HTTP 426 on the webserver route, and expose `apiFetch` without a webServer. Module tests compose the boot graph without HTTP. Desktop-app tests pin dist publication and the `app:desktop-surface` prompt. Protocol tests rewrite to loopback, serve `/api`, `/plugins`, and dist, and reject traversal. Host tests pin packed `resources/agent-presets` over the CLI config fallback. `boot()` appends every AggregateError member, not only `.cause`. The keyless host smoke boots the shipped desktop profile after a build and requires `host.describe` plus `session.create` through `apiFetch`. `dsh desktop` argument parsing is covered in the CLI args suite; the built CLI dump pins the desktop profile without a webserver.
