...
新增工作区包必须遵循[新增包清单](../../../../docs/cookbook/adding-a-package.md)：包脚手架、聚合注册、README 模型体验、invariant 伴随、双语配对与重新生成的目录。仓库里没有可供复制的、最小的工具形态清单演练，产品也没有供模型使用的纯函数、无状态文本测量工具。一个包可以同时满足两者：完整清单的最小示例，以及一个小型 `wc` 式能力。
