...
The Node half scans enabled Loader entries for web `dsh.client` packages, resolves each `exports["./client"]`, and hashes the built bundle into the boot graph. When a webServer is present it serves those files with their source maps under `/plugins` and injects `window.__DSH_BOOT__` through an index tap. Without a webServer the graph and `clientPath` remain available for a non-HTTP carrier such as the Electron `dsh:` protocol. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.
