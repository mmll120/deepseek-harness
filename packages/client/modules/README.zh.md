...
Node 侧会扫描已启用的 Loader 配置项以发现 web `dsh.client` 包，解析每个 `exports["./client"]`，并把构建后的 bundle 哈希写入启动图。存在 webServer 时，它会在 `/plugins` 下提供这些文件及其 sourcemap，并通过 index tap 注入 `window.__DSH_BOOT__`。没有 webServer 时，图与 `clientPath` 仍可供非 HTTP 载体使用，例如 Electron 的 `dsh:` 协议。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。
