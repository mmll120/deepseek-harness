...
| `@deepseek-ai/dsh-tool-text` | `text_stats` | `ctx.tools` | `tool/call`、`tool/result` | - | text_stats 是纯函数、无状态的测量，没有 Config，也不耦合会话或代理；本目录条目说明了行/单词/字符/字节的精确语义。 |
...
<a id="deepseek-aidsh-tool-text"></a>

## `@deepseek-ai/dsh-tool-text`

### `text_stats`

计算一段文本的行数、单词数、字符数、非空白字符数与 UTF-8 字节数。
...
来源：[`packages/text/tool-text/src/index.ts`](../packages/text/tool-text/src/index.ts)

text_stats 是纯函数、无状态的测量，没有 Config，也不耦合会话或代理；本目录条目说明了行/单词/字符/字节的精确语义。
