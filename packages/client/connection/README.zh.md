...
`/api/events.mux` 与 `/api/events.host` 各接受一条 WebSocket upgrade，并只向浏览器发送对应的 `ServerRequest` 文本消息；客户端不会在这些 socket 上发送业务数据。任一 socket 结束都会使当前 connection generation 失败并重建两条流，连接就绪仍要求两条 socket 均已打开且 `host.describe` HTTP 调用成功。Host teardown 会终止两条 socket、中止各自的 source，并等待 source 清理完成后再返回。普通网络 GET 这些路径在 HTTP webserver 路由上返回 426，不保留 SSE（Server-Sent Events）回退。Electron `dsh:` 协议使用的共享 `apiFetch` 会把这些 GET 转给 `toFetchHandler` 的 SSE 编解码；`ElectronApiClient` 沿用基类 SSE 读取。进程内载体也使用该编解码。
