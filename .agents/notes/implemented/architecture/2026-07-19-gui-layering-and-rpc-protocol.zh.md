...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop`（`@deepseek-ai/dsh-desktop`）在 Electron 主进程启动同一套 Host/Client 包，经特权 `dsh:` 协议加载同一份 frontend dist；不复用 `dsh-host-webserver`（[桌面客户端](2026-08-14-electron-desktop-client.md)）。
...
| `ElectronApiClient` | dsh-client-connection | 经特权 `dsh:` 协议的 `globalThis.fetch`；mux/host 沿用基类 SSE 读取 | Electron 渲染进程；物理边界见[桌面客户端](2026-08-14-electron-desktop-client.md) |
